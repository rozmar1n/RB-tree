...
TEST(RBTreeCliIterTest, ProcessesSampleInput) {
    std::istringstream input("k 10 k 20 q 8 31 q 6 9 k 30 k 40 q 15 40\n");
    std::ostringstream output;
    std::ostringstream error;

    const int exit_code = rb::run_cli_iter(input, output, error);
...
TEST(RBTreeCliIterTest, HandlesDescendingBoundsAndInclusiveLeft) {
